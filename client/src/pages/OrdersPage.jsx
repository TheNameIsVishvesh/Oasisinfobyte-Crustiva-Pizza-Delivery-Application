    ...
    <div className="min-h-screen bg-pizza-dark text-white py-12 px-6 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-8 z-10 relative">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight">Order Tracking Dashboard</h2>
            <p className="text-xs text-white/50 mt-1">Status changes sync automatically every 5s</p>
          </div>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-20 bg-white/5 rounded-[2rem] border border-dashed border-white/10 text-white/40 space-y-4">
            <ShoppingBag className="w-12 h-12 mx-auto text-white/20 animate-bounce" />
            <p className="text-sm">You haven't placed any gourmet orders yet.</p>
            <Link to="/" className="inline-block px-6 py-2.5 bg-pizza-primary text-white text-xs font-bold rounded-xl hover:shadow-glow transition-all">
          ...
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              return (
                <div key={order._id} className="glass-dark p-6 rounded-[2rem] border border-white/5 space-y-6">
                  {/* Card Header details */}
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-4">
                    <div>
                      <span className="text-xs text-white/40 block">Order Reference ID</span>
                      <code className="text-sm font-extrabold text-pizza-accent">{order._id}</code>
                    </div>

                    <div className="flex items-center gap-4">
                      <div>
                        <span className="text-xs text-white/40 block text-right">Payment Status</span>
                      ...
                      <div className="text-right">
                        <span className="text-xs text-white/40 block">Grand Total</span>
                        <span className="text-lg font-black text-pizza-primary">₹{order.totalAmount}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="py-4">
                    <div className="relative flex items-center justify-between">
                      ...
                      <div className="absolute left-0 right-0 h-1 bg-white/5 top-1/2 -translate-y-1/2 z-0 rounded-full"></div>
                      {/* Connection Active Progress Fill */}
                      <div 
                        className="absolute left-0 h-1 bg-pizza-primary top-1/2 -translate-y-1/2 z-0 rounded-full transition-all duration-1000"
                    ...
                    <div className="text-center sm:hidden mt-4">
                      <span className="text-xs uppercase tracking-widest font-black text-pizza-accent bg-pizza-accent/10 border border-pizza-accent/20 px-3 py-1 rounded-full">
                        Status: {order.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5 space-y-3">
                    <h5 className="text-xs font-semibold text-white/50 uppercase">Ingredients List & Specs:</h5>
                    <div className="divide-y divide-white/5 text-xs">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="py-2.5 first:pt-0 last:pb-0 flex justify-between items-start gap-4">
                          <div>
                            <span className="font-extrabold text-sm text-white/85">{item.name}</span>{' '}
                            <span className="text-[10px] text-pizza-accent font-bold bg-pizza-accent/10 px-2 py-0.5 rounded-full">{item.size}</span>
                          ...
                          <div className="text-right">
                            <span className="text-white/50 block">Qty: x{item.quantity}</span>
                            <span className="font-bold text-white/80">₹{item.price * item.quantity}</span>
                          </div>
                        </div>
                      ))}
                    </div>
